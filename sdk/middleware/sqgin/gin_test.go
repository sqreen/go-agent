	...
	t.Run("control flow", func(t *testing.T) {
		...
		t.Run("agent disabled", func(t *testing.T) {
			for _, tc := range []struct {
				...
				{
					name: "sqreen after a middleware - the middleware aborts",
					middlewares: []gin.HandlerFunc{
						func(c *gin.Context) {
							c.String(middlewareResponseStatus, middlewareResponseBody)
							c.Abort()
				...
				{
					name: "sqreen after a middleware - the handler aborts",
					middlewares: []gin.HandlerFunc{
						func(c *gin.Context) {
							c.Next()
							if !c.IsAborted() {
				...
				{
					name: "sqreen after a middleware - the middleware aborts after the handler",
					middlewares: []gin.HandlerFunc{
						func(c *gin.Context) {
							c.Next()
							if !c.IsAborted() {
